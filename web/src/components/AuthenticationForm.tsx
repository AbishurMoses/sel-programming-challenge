...
export default function AuthenticationForm({ onSuccess }: AuthenticationFormProps) {
  const [formData, setFormData] = useState(() => {
    const rememberedCreds = apiService.getCredentials();
    return {
      serverurl: "",
      username: rememberedCreds?.username ?? "",
      password: rememberedCreds?.password ?? "",
      checked: !!rememberedCreds,
    };
  });
  ...
  const handleSubmit = async (e: { preventDefault: () => void }) => {
    ...
    try {
      ...
      if (formData.checked) {
        apiService.setCredentials(formData.username, formData.password)
      }
      onSuccess()
    } catch (err) {
              ...
              <Field>
                <div className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(value) => setFormData({ ...formData, checked: value === "indeterminate" ? false : value })}
                  />
                  <FieldLabel
                  >
